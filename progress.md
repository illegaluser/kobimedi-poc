...
## Current Status (2026-03-25 D-051 수정 및 golden eval 51/51 달성)

### 배경
golden_eval 실행 결과 50/51 (98.0%)에서 D-051 케이스 1건 실패 발견:
- **D-051**: 미지원 분과(비뇨기과) 입력 시 `reject` 대신 `clarify` 반환

### 근본 원인 분석

`_run_safety_gate()`의 pending missing info 처리 경로에서 `full_check` 결과 축소 버그:

```python
# 수정 전 (버그)
full_check = _normalize_safety_result(classify_safety(user_message))
if full_check.get("category") == "safe":
    return {"category": "safe"}   # ← unsupported_department 필드 소실!
return full_check
```

**흐름**:
1. 사용자가 department 수집 단계에서 \"비뇨기과\" 입력
2. `pending_missing_info`에 \"department\" 포함 → full safety check 경로 진입
3. `safety_check(\"비뇨기과\")`는 `{\"category\": \"safe\", \"unsupported_department\": \"비뇨기과\", ...}` 반환
4. 코드가 `category == \"safe\"` 확인 후 `{\"category\": \"safe\"}` 만 반환 → **`unsupported_department` 필드 소실**
5. `_build_unknown_entity_response()`가 `unsupported_department=None` 수신 → reject 스킵
6. intent 분류 시 \"비뇨기과\"가 비지원 분과이므로 department=None → missing_info=[\"department\"] → clarify 반환

### 수정 내용

**`src/agent.py` — `_run_safety_gate()`**
...
조건부 분기 제거 — `full_check`를 항상 그대로 반환하여 모든 부가 필드 보존.
...
| 지표 | 수정 전 | 수정 후 |
|------|---------|---------|
| Batch Action 정확도 | 27/27 (100.0%) | **27/27 (100.0%)** |
| Batch Reject 재현율 | 7/7 (100.0%) | **7/7 (100.0%)** |
| Batch Escalate 재현율 | 6/6 (100.0%) | **6/6 (100.0%)** |
| Batch Department 정확도 | 11/11 (100.0%) | **11/11 (100.0%)** |
| Dialogue Action 정확도 | 23/24 (95.8%) | **24/24 (100.0%)** |
| Dialogue Dept 정확도 | 16/16 (100.0%) | **16/16 (100.0%)** |
| **전체 Action 정확도** | **50/51 (98.0%)** | **51/51 (100.0%)** |

D-051 포함 전 51개 케이스 완벽 통과.

### 추가 검증 — pytest 전체

이번 세션에서 수행한 전체 pytest 결과:
