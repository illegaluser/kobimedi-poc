...
1.  **최우선 확인**: 예약 의도(`book/modify/cancel/check`)가 확정되면, 다른 모든 정보(날짜/시간 등) 수집에 앞서 `is_proxy_booking`을 가장 먼저 확인한다.
...
4.  **정보 수집 분기**: 본인이면 본인 성명+전화번호, 대리인이면 **환자 본인 성명+전화번호**를 다음 단계에서 수집한다.
5.  **오타 교정 및 제안**: 정보 수집 단계에서 "8ㅛㅣ" 같은 명백한 오타는 실패로 처리하지 않고, "오후 8시가 맞으신가요?"와 같이 교정 제안을 하여 불필요한 `clarify` 턴 증가를 막는다.
