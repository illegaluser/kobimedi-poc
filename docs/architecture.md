...
1.  **최우선 확인**: 예약 의도(`book/modify/cancel/check`)가 확정되면, 성명/연락처 등 다른 모든 정보 수집에 앞서 반드시 `is_proxy_booking`을 가장 먼저 묻도록 대화 상태머신의 1순위로 강제한다.
2.  **명시적 대리 표현 감지**: 메시지에 “엄마 대신”, “아버지 예약” 같은 표현이 있으면 `is_proxy_booking=true`로 자동 전환한다.
3.  **명시적 질문**: 위 감지가 없으면 "예약하시는 분이 환자 본인이신가요, 아니면 가족이나 지인을 대신하여 예약하시는 건가요?"를 최우선으로 질문한다.
4.  **정보 수집 분기**: 본인이면 본인 성명+전화번호, 대리인이면 **환자 본인 성명+전화번호**를 다음 단계에서 수집한다. 이름과 전화번호가 동시 입력되었을 때 올바르게 둘 다 추출되도록 처리한다.
5.  **오타 교정 및 제안**: 정보 수집 단계에서 "8ㅛㅣ" 같은 명백한 오타는 실패 처리(턴 증가) 하지 않고, 교정 제안을 거치거나, 진행 중인 상태일 때는 턴 카운트를 초기화하여 성급한 에스컬레이션을 방지한다.
6.  **전화번호 우선 식별**: 수집된 `patient_contact`를 저장소 조회의 1차 키로 사용해 초진/재진 및 기존 예약을 판정한다.
