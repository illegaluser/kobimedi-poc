...
### 아키텍처 파이프라인
본 Agent는 LLM에 의존한 단순 챗봇이 아니라 아래의 엄격한 파이프라인 단계를 거칩니다.
...
6. **Persist & Response**: 최종 확인 절차 후 예약 확정 및 응답.
...
- **사용한 AI 도구**: Cursor, Gemini Code Assist
- **활용 패턴 (Planner - Implementer - Reviewer)**:
  - **Planner**: 과제 요구사항을 분석하여 `.ai/handoff/10_plan.md` 및 기능 명세 `features.json`의 체크리스트를 자동 생성.
  - **Implementer**: 도출된 TDD 기반 체크리스트를 따라 `src/agent.py` 및 `src/policy.py` 등 핵심 로직 작성 지원.
  - **Reviewer**: 에이전트를 통해 `golden_eval`의 멀티턴 시뮬레이션 결과에서 발견된 오분류 원인을 역추적하여, LLM 프롬프트 정제 및 파이썬 코드 버그를 자가 수정하도록 지시.
